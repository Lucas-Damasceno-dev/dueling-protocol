...
public class CustomJwtAuthGatewayFilterFactory extends AbstractGatewayFilterFactory<CustomJwtAuthGatewayFilterFactory.Config> {
    ...
    private static final Logger logger = LoggerFactory.getLogger(CustomJwtAuthGatewayFilterFactory.class);
        ...
        return (exchange, chain) -> {
            ...
            if (isWebSocket) {
                ...
                if (tokenParams != null && !tokenParams.isEmpty()) {
                    ...
                    logger.debug("Found token in query parameters for WebSocket: {}", 
            ...
            if (token == null) {
                logger.warn("No authentication token found in header or query parameters for path: {}", request.getURI().getPath());
            ...
            try {
                logger.debug("Attempting to validate JWT token for path: {}", request.getURI().getPath());
