...
# Script para iniciar o jogo Protocolo de Duelo completo (corrigido para ambiente local com Redis alternativo)
echo "=== Protocolo de Duelo - Iniciar Jogo (Corrigido para Local com Redis Local) ==="
...
# Iniciar o servidor com uma configuração que evite resolução de nomes Redis Sentinel
# Aqui usamos o perfil local-dev com os perfis server e default para manter os componentes necessários
...
echo ""
echo "NOTA: Este modo usa Redis local ao invés de Redis Sentinel para evitar problemas de resolução de hostname."